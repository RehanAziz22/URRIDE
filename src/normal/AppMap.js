...
import { ActivityIndicator, Alert, Button, Image, Modal, PermissionsAndroid, StyleSheet, Text, TouchableOpacity, View,BackHandler } from "react-native";
...
export default function AppMap() {
  ...
  const [bikes, setBikes] = useState(); // Use the dummy array
  const [rideStart, setRideStart] = useState(false); // Use the dummy array
  ...
  const [state, setState] = useState({
    pickuplocationCords: {
      latitude: user.location?.coordinates[0] || 24.860725675006115,
      longitude: user.location?.coordinates[1]||67.02430132223773,
  ...
  const fetchBikes = async () => {
    ...
    axios.get(`${BASE_URL}bikes`, { timeout: 10000 })
      .then((response) => {
        const filteredBikes = response.data.filter(bike => bike.markerVisible === true);

        // Set the filtered bikes to the state
  ...
  return bikes ? <View style={[mapStyles.container, styles.positionRelative]}>
    <MapView
      ref={mapRef}
    ...
    >
      ...
      <Marker coordinate={currentlocationCords} title={"Current location"}>
        <Image source={imagePath.icCurLoc} style={{ width: 50, height: 50 }} />
      </Marker>


      {bikes.map(bike => {

        return <Marker Marker key={bike._id} bike={bike} coordinate={{ latitude: bike.location.coordinates[0], longitude: bike.location.coordinates[1] }}
          onPress={() => {
            setSelectedBike(bike);
            console.log("Marker pressed", bike); // Debug log
            openModal();
          }}
        >
