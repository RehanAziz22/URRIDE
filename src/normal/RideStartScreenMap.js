...
import { Alert, Button, Image, Modal, PermissionsAndroid, StyleSheet, Text, TouchableOpacity, View,BackHandler } from "react-native";
...
export default function RideStartScreenMap() {
    ...
    const fetchBikeDetails = async () => {
        try {
            const response = await axios.get(`${BASE_URL}bike/${bike._id}`);
            const { success, message, data } = response.data
            if (success) {
                setBikeData(data);
                console.log('Bike details retrieved successfully:', data);
            } else {
                console.error('Failed to retrieve bike details:', message);
            }
        } catch (error) {
    ...
    const endRide = () => {
        ...
        let remainingFuel = (bikeData.fuelLevel / 100) * bike.fuelTankCapacity - fuelConsumed; // Remaining fuel in liters
        ...
        if (rideDetails) {
            ...
            axios
                .put(`${BASE_URL}bike/${bike._id}`, bikeobjToSend)
                .then((response) => {
                    const { success, message, data } = response.data;
                    if (success) {
    ...
    return <View style={[mapStyles.container, styles.positionRelative]}>
      ...
      <Marker coordinate={droplocationCords} image={imagePath.icGreenMarker} title={"DropLocations"}></Marker> */}
        ...
        <View style={[mapStyles.bottomCard, styles.py2, styles.alignItemsCenter, styles.shadow6, styles.w95]}>
            ...
            <TouchableOpacity onPress={endRide} style={[styles.w95, styles.btn, styles.AppBg1]}>
